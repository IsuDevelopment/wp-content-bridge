<?php
/**
 * Delete redirect ability adapter.
 *
...
/**
 * Maps the delete_redirect use case to a WordPress Ability.
 *
 * Requires `wpcb_manage_redirects`, and the named provider additionally
 * enforces its own backend authority.
 */
final readonly class DeleteRedirectAbilities {

	/**
	 * Creates the ability adapter.
	 *
	 * @param DeleteRedirect $use_case Redirect write use case.
	 */
	public function __construct( private DeleteRedirect $use_case ) {}
	...
	public function register_ability(): void {
		wp_register_ability(
			DeleteRedirect::ABILITY,
			array(
				'label'               => __( 'Delete redirect', 'wp-content-bridge' ),
				'description'         => __( 'Removes one redirect from a named provider. Confirmed by re-reading the provider, and not reversible from this plugin.', 'wp-content-bridge' ),
				'category'            => AbilityCategory::SLUG,
				'input_schema'        => AbilitySchemas::delete_redirect_input(),
				'output_schema'       => AbilitySchemas::delete_redirect_output(),
