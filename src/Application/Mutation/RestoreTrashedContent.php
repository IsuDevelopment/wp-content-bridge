<?php
/**
 * Restore-trashed-content use case.
 *
...
use IsuDev\WPContentBridge\Domain\Mutation\RestoreInput;
use Throwable;

/**
 * Restores eligible trashed content with concurrency and audit gates. The
 * mirror image of TrashContent — it shares every gate and never lets the
 * restored status reach `publish` or `future`.
 */
final readonly class RestoreTrashedContent {

	public const ABILITY = 'wp-content-bridge/restore-trashed-content';

	/**
	 * Creates the restore-trashed-content use case.
	 *
	...
	/**
	 * Executes one reversible restore mutation.
	 *
	 ...
	 * @throws MutationInvalidState When the target is not currently trashed.
	...
	public function execute( array $raw_input, int $user_id ): MutationResult {
		...
		try {
			$input            = RestoreInput::from_input( $raw_input );
			...
			if ( ! $this->access->allows( $target->post_type, ContentOperation::TRASH ) ) {
				throw new MutationForbidden( 'Restoring trashed content is not permitted for this type.' );
			}
			if ( 'trash' !== $target->status ) {
				throw new MutationInvalidState( 'Content cannot be restored from its current state.' );
			}
			...
			$result = $this->repository->untrash( $input->post_id );
