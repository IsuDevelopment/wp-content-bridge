<?php
/**
 * Update-block-attributes use case.
 *
...
use IsuDev\WPContentBridge\Domain\Mutation\BlockAttributesUpdate;
 ...
 * leaving every other block byte-identical by construction. Shares
 * update-block's gates exactly: policy, native capability (enforced by the
 * adapter), optimistic concurrency, and exactly one audit row per attempt. A
 * block path or attribute name is positional/field detail and never enters
 * the audit row, which records field names only (always ["content"]).
 */
final readonly class UpdateBlockAttributes {

	public const ABILITY = 'wp-content-bridge/update-block-attributes';
	 ...
	 * @param BlockTreeSplicer          $splicer    Path resolution and merge port.
	...
	/**
	 * Executes the update-block-attributes flow, recording exactly one audit
	 * row.
	 *
	 ...
	 * @throws BlockMismatch When the block at path is not of the expected type, or is a freeform node.
	...
	public function execute( array $raw_input, int $user_id ): MutationResult {
		...
		try {
			$update           = BlockAttributesUpdate::from_input( $raw_input );
			...
			if ( null === $lookup->block_name ) {
				throw new BlockMismatch( 'A freeform node has no attributes to merge.' );
			}

			$resulting_content = $this->splicer->merge_attributes( $snapshot['block_markup'], $update->path, $update->attributes );
