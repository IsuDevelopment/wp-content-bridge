...
use IsuDev\WPContentBridge\Domain\Mutation\CustomSchemaMutationResult;
use IsuDev\WPContentBridge\Domain\Mutation\CustomSchemaUpdate;
use IsuDev\WPContentBridge\Domain\Mutation\MutationResult;
use Throwable;

/**
 * Orchestrates a policy- and version-tested Custom Schema write.
 */
final readonly class UpdateCustomSchema {

	public const ABILITY = 'wp-content-bridge/update-custom-schema';
	 ...
	 * @param CustomSchemaWriter        $writer     Custom Schema write + re-read port.
	...
	public function __construct(
		...
		private CustomSchemaWriter $writer,
	 ...
	 * @throws Throwable Re-thrown validation or mutation failures.
	 */
	public function execute( array $raw_input, int $user_id ): CustomSchemaMutationResult {
		$post_id          = null;
		$post_type        = null;
		$expected_version = null;

		try {
			$update           = CustomSchemaUpdate::from_input( $raw_input );
			...
			if ( ! $this->access->allows( $post_type, ContentOperation::UPDATE_SEO ) ) {
				throw new MutationForbidden( 'Custom Schema updates are not permitted for this type.' );
			}
			...
			$result   = new CustomSchemaMutationResult( $mutation, $effective );
	...
	private function classify( Throwable $error ): array {
		...
		if ( $error instanceof CustomSchemaUnavailable ) {
			return array( 'invalid', $error->error_code() );
		}
